 ...
 */
router.post('/:id/claim', requireAuth, authorizeRoles('traveler'), validateParams(idSchema), async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
 ...
 */
router.post('/:id/approve', requireAuth, authorizeRoles('shopper'), validateParams(idSchema), async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
      return res.status(400).json({
 ...
 */
router.post('/:id/cancel', requireAuth, authorizeRoles('shopper', 'traveler'), validateParams(idSchema), async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
      return res.status(400).json({
    ...
    const match = await bookingService.cancelMatchDuringCooldown(userObjectId, {
      matchId: id,
      reason,
    });
